...
static SetNode node_rotate_left(SetNode node) {
	...
	node->size = ((node->left != NULL) ? node->left->size : 0) + ((left_subtree != NULL) ? left_subtree->size : 0) + 1;
...
static SetNode node_rotate_right(SetNode node) {
	...
	node->size = ((node->right != NULL) ? node->right->size : 0) + ((left_right != NULL) ? left_right->size : 0) + 1;
...
static SetNode node_insert(SetNode node, CompareFunc compare, Pointer value, bool* inserted, Pointer* old_value) {
	...
	if (compare_res == 0) {
		...
		node->size++;	// αυξάνουμε τα μεγέθη στο "κατέβασμα"
		*inserted = false;
	...
	if (*inserted == false) {
		node->size--;	// αν δεν έγινε insert ξαναμειώνουμε τα μεγέθη στο "ανέβασμα"
	}
...
static void node_return_from_to(Set set, SetNode node, Pointer from, Pointer to, List list) {
	...
	if (set->compare(node->value, to) < 0) {
        node_return_from_to(set, node->right, from, to, list);
    }
    if ((set->compare(node->value, from) >= 0) && (set->compare(node->value, to) <= 0)) {
    	list_insert_next(list, LIST_BOF, node->value);
    }
    if (set->compare(node->value, from) > 0) {
